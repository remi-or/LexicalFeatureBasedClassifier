...
class DistilledCamembertForSequenceClassification(Module):
    ...
    def distillation_loss(
        ...
        loss = torch.nn.CosineEmbeddingLoss()(
            teacher_pooler_outputs,
            student_pooler_outputs,
            torch.ones(teacher_pooler_outputs.size()[0], device=self.device),
        ) * self.loss_weights[0]
        # Classification loss
        ...
        teacher_pooler_outputs = (teacher_pooler_outputs / self.temperature).softmax(1)
        student_pooler_outputs = student_pooler_outputs / self.temperature
        loss = loss + torch.nn.CrossEntropyLoss()(
            student_pooler_outputs,
            teacher_pooler_outputs,
        )* self.loss_weights[2]
    ...
    def forward(
        ...
        with torch.no_grad():
            teacher_pooler_outputs = self.get_pooler_outputs(
            ...
            teacher_classifier_outputs = self.classfiy(
                pooler_outputs=teacher_pooler_outputs,
                teacher=True,
            )
        # Student part, with gradient (except if already in a no gradient context)
        try:
            student_pooler_outputs = self.get_pooler_outputs(
                    input_ids=dataset_batch.X[0].to(self.device),
                    attention_mask=dataset_batch.X[1].to(self.device),
                    teacher=False,
                )
        ...
        if with_loss:
            loss = self.distillation_loss(
                teacher_pooler_outputs[:, 0, :],
                student_pooler_outputs[:, 0, :],

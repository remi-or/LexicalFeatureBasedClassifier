...
class Module(torch.nn.Module):
    ...
    def fit(
        ...
        training_dataset.reset_randomness()

        if validation_dataset is not None:
            number_of_validation_batches = validation_dataset.get_number_of_batches(**dataset_kwargs)
            validation_dataset.reset_randomness()
        else:
        ...
        for i_epoch in range(epochs):
            ...
            if callbacks is not None:
                if callbacks(self, optimizer, logger):
                    if verbose:
